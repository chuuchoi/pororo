...
if os.path.exists(autosave_file):
    result_df = pd.read_csv(autosave_file, encoding="ansi")
...
total_files = sum(1 for file in os.listdir(os.path.join(folder_path, "labeling")) if file.endswith(".json") and file not in processed_files_set)
processed_files = 0;
    ...
    for file in files:
        if file.endswith(".json") and file not in processed_files_set:
            ...
            if processed_files % 10 == 0:
                result_df.to_csv(autosave_file, index=False, encoding="ansi", errors='ignore')
...
# 마지막 결과 저장
result_df.to_csv("auto_validation_final.csv", index=False, encoding="ansi", errors='ignore')
