...
# CSV 파일 불러오기 또는 초기화
final_autosave_file = 'auto_validation_autosave_0.csv'
autosave_file = "auto_validation_autosave_"
save_idx = 0
...
#suspected_files.csv 초기화
col_suspected = ["라벨링 문제 의심 100%","90% 이상","80% 이상"]
suspected_df = pd.DataFrame(columns=col_suspected)
...
for root, dirs, files in os.walk(os.path.join(folder_path, "labeling"), topdown=True):
    for file in files:
        if file.endswith(".json") and file not in processed_files_set:
            ...
            # image_folder_path = r"/media/ubuntu2/6281-911C/image/"
            image_file_path = ''
            ...
            # 이미지 파일 열기
            try:
                if file.replace('.json', '') not in image_file_path:
            ...
            for label_info in label_data["annotations"]["ocr_labels"]:
                if label_info["type"] == "bbox":
                    ...
                    try:
                        ...
                        suspected_df = suspected_df._append({
                            "라벨링 문제 의심 100%": f"{file}의 Bounding Box 영역 OCR 처리 중 오류 발생:/에러{e}",
                            "90% 이상":"",
                            "80% 이상":"",
                        }, ignore_index=True)
                        continue
                    ...
                    if len(result_df) == 900000:
                        final_autosave_file = autosave_file + str(save_idx) + '.csv'
                        save_idx += 1
                        result_df.to_csv(final_autosave_file, index=False, encoding="utf-8", errors='ignore')
                        print(f"중간 저장 완료: {final_autosave_file}")
                        result_df = pd.DataFrame(columns=columns)
                    ...
                    if cet=='': #특수문자만 있어서 패스뜰경우 패스를 FAIL로 변경
                        one_file_result["ocr_results"].append("FAIL")
                    else:
                        one_file_result["ocr_results"].append("PASS" if l_distance < 1 else "FAIL") #오타 1개 허용 안함
                    one_file_result["bbox_results"].append("PASS" if iou > 0.8 else "FAIL")
            ...
            #FAIL 수가 80프로 이상인 경우 라벨링 문제 의심으로
            fail_ratio_ocr = len([f for f in one_file_result["ocr_results"] if f=="FAIL"])/len(one_file_result["ocr_results"])
            fail_ratio_bbox = len([f for f in one_file_result["bbox_results"] if f=="FAIL"])/len(one_file_result["bbox_results"])
            if fail_ratio_ocr==1 or fail_ratio_bbox == 1:
                suspected_df = suspected_df._append({
                    "라벨링 문제 의심 100%": os.path.basename(image_file_path),
                    "90% 이상":"",
                    "80% 이상":"",
                }, ignore_index=True)
            elif fail_ratio_ocr>=0.9 or fail_ratio_bbox >= 0.9:
                suspected_df = suspected_df._append({
                    "라벨링 문제 의심 100%": "",
                    "90% 이상":os.path.basename(image_file_path),
                    "80% 이상":"",
                }, ignore_index=True)
            elif fail_ratio_ocr>=0.8 or fail_ratio_bbox >= 0.8:
                suspected_df = suspected_df._append({
                    "라벨링 문제 의심 100%":"",
                    "90% 이상":"",
                    "80% 이상":os.path.basename(image_file_path),
                }, ignore_index=True)
            one_file_result={ #initialization
                "ocr_results":[],
                "bbox_results":[],
            }
...
final_autosave_file = autosave_file + str(save_idx) + '.csv'
result_df.to_csv(final_autosave_file, index=False, encoding="utf-8", errors='ignore')
suspected_df.to_csv('suspected_files.csv',index=False, encoding="utf-8", errors='ignore')
